...
3、读取单个地址可带入功能码，如50_4，表示地址为50、功能码为4";
            ...
            //{
                try
                {
                    ...
                    if (result.IsSucceed)
                    {
                        ...
                        che_plcadd.Enabled = false;
                        AppendText($"连接成功\t\t\t\t耗时：{result.TimeConsuming}ms");
                        ControlEnabledFalse();
                        open_change_event.Invoke("温箱", true);
                    //将station number传出去
        ...
        public void CloseAll()
        {
            ...
            but_close_server_Click(null,null);
        }
        private void but_close_Click(object sender, EventArgs e)
        {
            ...
            but_close.Enabled = false;
            but_sendData.Enabled = false;
            che_plcadd.Enabled = true;
            AppendText($"连接关闭");
            ControlEnabledTrue();
            open_change_event.Invoke("温箱", false);
        }
        ...
        public byte station_Number = 0x01;
